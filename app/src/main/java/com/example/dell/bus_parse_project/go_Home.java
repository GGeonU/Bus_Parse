...
public class go_Home extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        String tablename = "Go_Home";

        String arsId, stId;


        SQLiteDatabase database = this.openOrCreateDatabase(tablename, MODE_PRIVATE, null);
        boolean checkTable = checkTable(database);

        Log.d("cc", String.valueOf(checkTable));
        ...
        if(checkTable) {
            ...
            Toast.makeText(getApplicationContext(), "등록된 정류장이 없습니다.", Toast.LENGTH_LONG).show();
        }
    ...
    private boolean checkTable(SQLiteDatabase database) {
        ...
        Cursor cursor = database.rawQuery("SELECT name FROM sqlite_master WHERE type='table' AND name ='Go_Home'" , null);
        cursor.moveToFirst();
